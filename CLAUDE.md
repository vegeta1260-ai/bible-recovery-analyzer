...
- **字型 self-host（`web/public/fonts/`，537 woff2 + fonts.css，~25MB）**：去除 Google Fonts 外部依賴。`fonts.css` 用**相對 url**（與 base path 無關），保留 Google 原本的 unicode-range 分塊，瀏覽器只抓用到的字（含 runtime 恢復本經文字，故 CJK 全分塊都要保留、不可砍）。BaseLayout 以 `<link rel=preload as=style>` 非阻塞載入。⚠️ 要更新字型：抓 `https://fonts.googleapis.com/css2?family=...`（用桌面 Chrome UA 才給 woff2）→ 解析每個 @font-face 的 woff2 url + unicode-range → 全下載到 public/fonts → 改寫 src 為相對檔名。smoke 有「無 Google 引用」回歸 gate。
