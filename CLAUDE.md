...
- **逐章「較深概要」（`web/src/data/chapter-summaries/*.json`，66 卷 / 1189 章全覆蓋）**：LLM 逐卷生成的 ~250–400 字繁中概要，渲染於逐章頁「本章概要」（優先於舊的 ~100 字 `chapter-outlines`，無則退回）。內容為自撰、不含恢復本版權經文。每檔 key 為章號字串、value 為概要字串。smoke 有覆蓋 gate（66 卷 + Gen1 內容）。
