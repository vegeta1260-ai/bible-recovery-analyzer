...
純靜態前端，部署於自架主機 `bible.weiqi.kids`（GitHub Actions 以 rsync over SSH 發佈）。聖經原文字義研究工具：
...
- 視覺特效（依經文主題觸發）+ 環境音樂（2 首公共領域素歌，Web Audio 程序化音效）
- OKLCH 暖色系 + 動態字體 + 響應式 + 深色模式
- SEO/AEO：**15,392 頁**預渲染 HTML + JSON-LD + OG tags + `sitemap.xml`（@astrojs/sitemap，含全部頁面）+ `robots.txt`

線上網址：https://bible.weiqi.kids/
（舊 GitHub Pages 站 `https://vegeta1260-ai.github.io/bible-recovery-analyzer/` 內容凍結於 2026-06-19，待新站驗證後轉址退場）
...
| 音效 | Howler.js | 2.x | 環境音樂播放、淡入淡出（首次播放時動態載入） |
| 程序化音效 | Web Audio API | 原生 | 翻頁聲、鐘聲（~3KB，無外部依賴） |
| 樣式 | CSS Modules + OKLCH | — | Design tokens，暖色系亮色/深色雙模式 |
| 測試 | Vitest | 4.x | 單元測試 + build 煙霧測試（dist/ 產物 gate）+ 部署後煙霧測試 |
...
| 字體 | Noto Serif/Sans TC, JetBrains Mono | self-host（`public/fonts/`，537 woff2） | 中文 + 等寬；原文（希伯來/希臘）暫用系統字型 |
...
├── public/
│   ├── audio/                 # 環境音樂 MP3（公共領域素歌；musicManager 將 8 種書卷類型映射到這 2 軌）
│   │   ├── ambient-chant.mp3       # 素歌 A
│   │   └── ambient-rorate.mp3      # 素歌 B（Rorate caeli）
│   ├── data/
...
│   ├── fonts/                 # self-host 字體（Noto Sans/Serif TC + JetBrains Mono，537 woff2 + fonts.css）
...
│   ├── effects/               # 視覺特效 React 元件（Router + 依經文主題觸發的特效）
│   ├── audio/                 # 音樂/音效控制模組
│   ├── data/                  # Build 時嵌入的 JSON
│   │   ├── bookMap.json       # 66 卷書對照表
│   │   ├── lexicon.json       # 14,197 筆 Strong's（build 時嵌入；全站唯一來源）
│   │   ├── analyticalCodes.json # 分析碼圖例 + 文法注記
│   │   ├── lastmod.json       # 內容真實最後變更日 manifest（scripts/build-lastmod.mjs 產生）
│   │   ├── chapter-summaries/ # 逐章「較深概要」66 檔 / 1,189 章（渲染於章頁「本章概要」）
│   │   └── chapter-meditations/ # 逐章默想/禱讀 66 檔 / 1,189 章
...
│   ├── build-lastmod.mjs      # 產 lastmod.json（sitemap/Article 真實日期；資料檔變更後重跑）
│   ├── smoke-test-build.sh    # build 後煙霧測試（查 dist/ 產物，任一失敗則 CI 不部署）
│   └── smoke-test-deployed.sh # 部署後煙霧測試（查線上版本 + LSM API live；CI 部署後自動跑）
└── tests/                     # Vitest 單元測試
```
...
| `chapter-summaries/{osis}.json` | 66 檔 / 1,189 章 | 逐章頁「本章概要」：LLM 逐卷生成的 ~250–400 字繁中概要，自撰、受版權約束（禁貼近恢復本綱目）。（舊的 ~100 字 `chapter-outlines` 已退役移除） |
| `chapter-meditations/{osis}.json` | 66 檔 / 1,189 章 | 逐章「默想/禱讀」段落 |
| `lastmod.json` | 66 卷 + 3 項 | 內容真實最後變更日（git commit 日）；sitemap 與章頁 Article.dateModified 讀此檔，**資料檔變更後須重跑 `scripts/build-lastmod.mjs`** |
...
| `analyzer.ts` | `getVerseTokens()`, `lookupWord()`, `lookupLemma()`, `loadBookTokens()`, `scanBooks()` | 非同步（按書卷動態載入 + IndexedDB 版本化快取，併發掃卷上限 4） |
...
1. 修改程式碼
2. 本地測試：npm test
3. 本地預覽：npm run dev → http://localhost:4321/
4. （建議）本地 build 煙霧測試：npm run build && npm run test:smoke（push 前先攔壞 build）
5. commit + push main
6. GitHub Actions 自動 test + check:recovery + build + smoke → rsync 部署 → 部署後煙霧測試
   （任一步失敗則不部署；部署後驗證失敗會讓 run 轉紅通知）
7. 確認線上：https://bible.weiqi.kids/
```
...
3. 音檔命名規則：ambient-{名稱}.mp3（現存 chant / rorate 兩軌，8 種書卷類型映射到這 2 軌）
4. 新增音軌時更新 web/src/audio/musicManager.ts 的 TRACK_MAP 和 BOOK_TYPE_MAP，
   並同步 smoke-test-build.sh / smoke-test-deployed.sh 的音檔 gate（只 gate 實際被引用的檔）
...
2. 執行 ETL 鏈（⚠️ 順序固定、一步不可漏；下游腳本已有「長 key 輸入」防呆，漏跑 compress 會被擋下）：
   ...
   python3 web/scripts/remap-joel-mal-versification.py  # ⚠️ 必跑：Joel/Mal 章層級重對映
   python3 web/scripts/remap-versification.py --apply   # ⚠️ 必跑：24 卷節層級重對映（冪等：指紋偵測已套用即 SKIP）
   python3 web/scripts/backfill-ot-lemma.py             # ⚠️ 必跑：OT lemma 正規化為字典形
   python3 web/scripts/build-strongs-occurrences.py     # ⚠️ 必跑：重建 Strong's 出現索引
   node web/scripts/build-lastmod.mjs                    # 重算 lastmod manifest（token 檔變了）
3. 驗證分節：node web/scripts/scan-versification.mjs（review 應只剩新約 4 章）
4. npm run build && npm run test:smoke 確認（含 token 有效內容 + versification 指紋 gate）
5. commit + push
```
...
### SOP 11：更新逐章頁「本章概要」（chapter-summaries）

```
1. 來源：web/src/data/chapter-summaries/{osis}.json（key=章號字串 → ~250–400 字概要）
   66 卷 1,189 章全覆蓋（舊的 ~100 字 chapter-outlines 已退役移除，無退路分支）。
2. 撰寫約束（硬性）：自撰、純描述性、神學中性、繁中、
   禁抄襲或貼近恢復本（Recovery Version）章首綱目與註解。
3. 大量產出用多個 agent 平行（一卷一檔，互不衝突）。
4. 章數必須對齊「恢復本章節」（token 已重對映：Joel 3 章、Mal 4 章）：
   node -e "const d=require('./src/data/chapter-summaries/Xxx.json');console.log(Object.keys(d).length)"
5. node scripts/build-lastmod.mjs（概要變更 → 對應章頁 lastmod/dateModified 更新）
6. npm test && npm run build && npm run test:smoke（content-completeness 有 66 卷逐章非空 gate）
7. commit + push
```
